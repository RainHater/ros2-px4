#include "task_executor/action/set_offboard_mode_action.h"
#include "utilities/topic_name.hpp"

SetOffboardModeService::SetOffboardModeService()
    :rclcpp::Node("set_offboard_mode_server")
{
    RCLCPP_INFO(get_logger(), "set_offboard_mode_server 节点启动...");
}

void SetOffboardModeService::initialize(){
    init_publisher();
    init_subscription();
    init_action();
}

void SetOffboardModeService::init_publisher(){
    m_set_offboard_mode_pub = create_publisher<common_msgs::msg::ArmOffboardStatus>(
        topic_pub::SET_OFFBOARD_MODE, 10);
}

void SetOffboardModeService::init_subscription(){
...
void SetOffboardModeService::init_action(){
...
void SetOffboardModeService::execute(
